...
####### extract data
bioCurrentData <- terra::extract(bioLayersCurrent, PrCWpoints[, c("Long", "Lat")]) 

bioCurrentData$Long <- PrCWpoints$Long
bioCurrentData$Lat <- PrCWpoints$Lat
...
bioFutureData <- terra::extract(bioLayersFuture, PrCWpoints[, c("Long", "Lat")]) 
# remove layers with lots of NAs
#which(colSums(is.na(bioFutureData)) > 11)
bioFutureData$Long <- PrCWpoints$Long
bioFutureData$Lat <- PrCWpoints$Lat
...
fullCurrentData$Presence <- c(rep(1, nrow(PrCWdata)), rep(0,nrow(bg)))
...
varImpPlot(rf3) #%incMSE: bio13, bio18, bio4, incnodepurity: npp, gsp

erf3 <- dismo::evaluate(p=testpres, a=testbackg, model=rf3)
erf3 #AUC=0.978; cor: 0.87; max TPR+TNR at : 0.71

# full model
rfFull <- randomForest(Extant ~ ., data=fullData, na.action = na.exclude, importance=T) #, mtry=2
varImpPlot(rfFull)
...
#prFull
#prFuture
terra::writeRaster(prFull, "PrCW_Current_RF-SDM.tif")
terra::writeRaster(prFuture, "PrCW_Future_RF-SDM.tif")
# Data Frames
# rfDFCurrent
# rfDFFuture
write.csv(rfDFCurrent, "PrCW_Current_RF-SDM.csv")
write.csv(rfDFFuture, "PrCW_Future_RF-SDM.csv")
