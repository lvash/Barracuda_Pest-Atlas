...
### Species: Platynota idaeusalis Tufted Apple Bud Moth

TABMdata <- read.csv("GBIF_Occurrence/Tufted Apple Bud Moth/occurrence_TABM.csv", sep=",", header=T)
TABMpoints <- TABMdata %>%
...
####### extract data
bioCurrentData <- terra::extract(bioLayersCurrent, TABMpoints[, c("Long", "Lat")]) 

bioCurrentData$Long <- TABMpoints$Long
bioCurrentData$Lat <- TABMpoints$Lat
...
bioFutureData <- terra::extract(bioLayersFuture, TABMpoints[, c("Long", "Lat")]) 
# remove layers with lots of NAs
#which(colSums(is.na(bioFutureData)) > 11)
bioFutureData$Long <- TABMpoints$Long
bioFutureData$Lat <- TABMpoints$Lat
...
fullCurrentData$Presence <- c(rep(1, nrow(TABMdata)), rep(0,nrow(bg)))
...
varImpPlot(rf3) #%incMSE: bio18, bio13, bio15, bio16, gdd10, bio19, bio4 

erf3 <- dismo::evaluate(p=testpres, a=testbackg, model=rf3)
erf3 #AUC=0.958; cor: 0.796; max TPR+TNR at : 0.0922

## important variables
rf4 <- randomForest(Extant ~ bio18 + bio13+ bio15 + bio16 + gdd10 + bio19 + bio4, data=envtrain, na.action = na.exclude, importance=T)
varImpPlot(rf4) #
erf4 <- dismo::evaluate(p=testpres, a=testbackg, model=rf4)
erf4 #AUC= 0.95; cor = 0.79; max TPR+TNR: 0.0995


# full model
rfFull <- randomForest(Extant ~ gdd10 + bio18 + bio13+ bio15 + bio16 + bio19 + bio4, data=fullData, na.action = na.exclude, importance=T, do.trace = 100) #, mtry=2
varImpPlot(rfFull)
...
#prFull
#prFuture
terra::writeRaster(prFull, "TABM_Current_RF-SDM.tif")
terra::writeRaster(prFuture, "TABM_Future_RF-SDM.tif")
# Data Frames
# rfDFCurrent
# rfDFFuture
#write.csv(rfDFCurrent, "TABM_Current_RF-SDM.csv")
#write.csv(rfDFFuture, "TABM_Future_RF-SDM.csv")
